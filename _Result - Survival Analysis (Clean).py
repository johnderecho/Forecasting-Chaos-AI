...
PATHS = {
    "Lorenz": {
        "BiTCN": BASE_DIR / "Results" / "Nixtla LC - CNN BiTCN.json",
        "TCN": BASE_DIR / "Results" / "Nixtla LC - CNN TCN.json",
        "NBEATS": BASE_DIR / "Results" / "Nixtla LC - MLP NBEATS.json",
        "NHITS": BASE_DIR / "Results" / "Nixtla LC - MLP NHITS.json",
        "TSMixer": BASE_DIR / "Results" / "Nixtla LC - MLP TSMixer.json",
        "SOFTS": BASE_DIR / "Results" / "Nixtla LC - MLP SOFTS.json",
        "MLP Vanilla": BASE_DIR / "Results" / "Nixtla LC - MLP Vanilla.json",
        "DeepNPTS": BASE_DIR / "Results" / "Nixtla LC - MLP DeepNPTS.json",
        "StemGNN": BASE_DIR / "Results" / "Nixtla LC - GNN StemGNN.json",
        "NLinear": BASE_DIR / "Results" / "Nixtla LC - MLP NLinear.json",
        "Vanilla RNN": BASE_DIR / "Results" / "Nixtla LC - RNN Vanilla.json",
        "KAN RMOK": BASE_DIR / "Results" / "Nixtla LC - KAN RMoK.json",

    },
}

GROUND_TRUTH_PATH = r"C:\Users\Windows\Desktop\Derecho - Thesis\dysts\data\test_multivariate__pts_per_period_100__periods_12.json.gz"
...
for equation_name, model_dict in PATHS.items():
    ...
    for model_name, path in model_dict.items():
        ...
        # 🔥 FIX: unwrap equation-level JSON properly
        if isinstance(data, dict) and equation_name in data:
            inner = data[equation_name]

            # extract all models inside (BiTCN_final, etc.)
            for inner_model_name, inner_block in inner.items():

                # only keep valid prediction entries
                if isinstance(inner_block, dict) and "prediction" in inner_block:
                    results[equation_name][inner_model_name] = inner_block

        else:
            print(f"{path} does not contain key '{equation_name}', skipping.")
