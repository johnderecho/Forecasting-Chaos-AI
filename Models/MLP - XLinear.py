...
# ================= CONFIG =================
input_train_path = r"C:\Users\Windows 11\Desktop\Thesis - GPU\data\train_multivariate__pts_per_period_100__periods_12_noise.json.gz"   # use dynamic finder
input_test_path  = r"C:\Users\Windows 11\Desktop\Thesis - GPU\data\test_multivariate__pts_per_period_100__periods_12_noise.json.gz"    # use dynamic finder
output_path =      r"C:\Users\Windows 11\Desktop\Thesis - GPU\Results\LN - XLinear.json"                                                             # use dynamic finder
...
RANDOM_SEED = 42                                                                                                             # Number of series to train on TSMixer_final
...
def make_objective(train_past, train_future, n_dims):
    ...
    def objective(trial):
        ...
        # Filter only valid NLinear args
        sig = inspect.signature(XLinear.__init__)
        ...
        try:
            model = NeuralForecast(models=[XLinear(**params)], freq=1)
            model.fit(df_train)
            fcst = model.predict()
...
for attractor in attractors:
    ...
    study = optuna.create_study(
        direction="minimize",
        pruner=PRUNER,
        study_name=f"XLinear_{attractor}",
        storage=STORAGE,
        load_if_exists=True
    )
    ...
    model = NeuralForecast(models=[XLinear(**best_params)], freq=1)
    model.fit(df_train)
    ...
    all_results[attractor]["XLinear_final"] = {
