...
output_path = r"C:\Users\Windows\Desktop\Thesis - GPU\results\Transformers TFT Base - 1.json"                                                             # use dynamic finder
...
def make_objective(train_past, train_future, n_dims):
    ...
    def objective(trial):
        ...
        # Filter only valid TFT args
        sig = inspect.signature(TFT.__init__)
        ...
        try:
            model = NeuralForecast(models=[TFT(**params)], freq=1)
            model.fit(df_train)
            fcst = model.predict()
...
for attractor in attractors:
    ...
    model = NeuralForecast(models=[TFT(**best_params)], freq=1)
    model.fit(df_train)
    ...
    all_results[attractor]["TFT_final"] = {
...
def plot_phase_space_3d(y_true, y_pred=None, attractor_name=""):
    ...
    if y_pred is not None:
        ax.plot(y_pred[:, 0], y_pred[:, 1], y_pred[:, 2],
                label='TFT Prediction', color='crimson', alpha=0.9,
...
def main():
    ...
    for attractor_name, attractor_res in results.items():
        ...
        if "TFT_final" not in attractor_res:
            print(f"  No 'TFT_final' for {attractor_name}, skipping.")
            continue
        ...
        # Predictions
        y_pred = np.array(attractor_res["TFT_final"]["prediction"])
