...
output_path = r"C:\Users\Windows\Desktop\Thesis - GPU\results\MLP SOFTS Base - 10.json"                                                             # use dynamic finder
...
def make_objective(train_past, train_future, n_dims):
    ...
    def objective(trial):
        ...
        # Filter only valid SOFTS args
        sig = inspect.signature(SOFTS.__init__)
        ...
        try:
            model = NeuralForecast(models=[SOFTS(**params)], freq=1)
            model.fit(df_train)
            fcst = model.predict()
...
for attractor in attractors:
    ...
    model = NeuralForecast(models=[SOFTS(**best_params)], freq=1)
    model.fit(df_train)
    ...
    all_results[attractor]["SOFTS_final"] = {
...
def plot_phase_space_3d(y_true, y_pred=None, attractor_name=""):
    ...
    if y_pred is not None:
        ax.plot(y_pred[:, 0], y_pred[:, 1], y_pred[:, 2],
                label='SOFTS Prediction', color='crimson', alpha=0.9,
...
def main():
    ...
    for attractor_name, attractor_res in results.items():
        ...
        if "SOFTS_final" not in attractor_res:
            print(f"  No 'SOFTS_final' for {attractor_name}, skipping.")
            continue
        ...
        # Predictions
        y_pred = np.array(attractor_res["SOFTS_final"]["prediction"])
