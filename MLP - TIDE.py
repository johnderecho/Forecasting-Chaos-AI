...
def make_objective(train_past, train_future, n_dims):
    ...
    def objective(trial):
        ...
        try:
            model = NeuralForecast(models=[TiDE(**params)], freq=1)
            model.fit(df_train)
            fcst = model.predict()
...
for attractor in attractors:
    ...
    model = NeuralForecast(models=[TiDE(**best_params)], freq=1)
    model.fit(df_train)
...
def plot_phase_space_3d(y_true, y_pred=None, attractor_name=""):
    ...
    if y_pred is not None:
        ax.plot(y_pred[:, 0], y_pred[:, 1], y_pred[:, 2],
                label='TiDE Prediction', color='crimson', alpha=0.9,
...
def main():
    ...
    for attractor_name, attractor_res in results.items():
        ...
        if "TiDE_final" not in attractor_res:
            print(f"  No 'TiDE_final' for {attractor_name}, skipping.")
            continue
        ...
        # Predictions
        y_pred = np.array(attractor_res["TiDE_final"]["prediction"])
