...
def make_objective(train_past, train_future, n_dims):
    ...
    def objective(trial):
        ...
        # Filter only valid DeepNPTS args
        sig = inspect.signature(DeepNPTS.__init__)
        ...
        try:
            model = NeuralForecast(models=[DeepNPTS(**params)], freq=1)
            model.fit(df_train)
            fcst = model.predict()
...
for attractor in attractors:
    ...
    model = NeuralForecast(models=[DeepNPTS(**best_params)], freq=1)
    model.fit(df_train)
...
def plot_phase_space_3d(y_true, y_pred=None, attractor_name=""):
    ...
    if y_pred is not None:
        ax.plot(y_pred[:, 0], y_pred[:, 1], y_pred[:, 2],
                label='DeepNPTS Prediction', color='crimson', alpha=0.9,
